...
class Generator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims):
        ...
        ###  TODO: BY NISHANT GAUTAM 210832761
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims=1):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(1024, 512),
            nn.LeakyReLU(0.2),
           nn.Dropout(0.3)  # TO BE REMOVED AT LATER STAGES 
        )
        # Third Layer
        ...
        # Final Output Layer -  Model Output
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    grd = 5  # GRID SIZE GIVEN
    img_size = 28
    outputs = G_net(z_)

    c = np.empty((img_size * grd, img_size * grd))  #EMPTY GRID INITIALIZED TO STORE OUTPUT IMAGE
    for i in range(grd):
        for j in range(grd):
            c[i * img_size:(i + 1) * img_size, j * img_size:(j + 1) * img_size] = outputs.cpu().detach().numpy()[i * 5 + j].reshape([img_size, img_size])   #SAVING OUTPUT IMAGES IN THE GRID
    plt.figure(figsize=(grd, grd))  #GRID VISUALIZATION
    plt.imshow(c, origin="upper", cmap="gray") # GRID SHOW
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        path = image_save_dir + '/MNIST_GAN_2' + str(epoch + 1) + '.png'
